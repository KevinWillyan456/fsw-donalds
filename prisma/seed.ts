...
const main = async () => {
  await prismaClient.$transaction(async (tx: any) => {
    ...
    for (const category of menuCategories) {
      categories[category.name] = await tx.menuCategory.create({
        data: {
          name: category.name,
    ...
    const products = [
      {
        ...
        menuCategoryId: categories['Combos'].id,
        restaurantId: restaurant.id,
        ingredients: [
          'Pão com gergilim',
      ...
      },
      {
        ...
        menuCategoryId: categories['Lanches'].id,
        restaurantId: restaurant.id,
        ingredients: [
          'Pão com gergilim',
          'Hambúrguer de carne 100% bovina',
      ...
      },
      {
        ...
        menuCategoryId: categories['Fritas'].id,
        restaurantId: restaurant.id,
        ingredients: []
      },
      {
      ...
      },
      {
        ...
        menuCategoryId: categories['Bebidas'].id,
        restaurantId: restaurant.id,
        ingredients: []
      },
      {
      ...
      },
      {
        ...
        menuCategoryId: categories['Sobremesas'].id,
        restaurantId: restaurant.id,
        ingredients: []
      },
      {
    ...
    await tx.product.createMany({
      data: products.map(({ ingredients, ...product }) => product)
    })

    // Buscar os produtos recém-criados para associar os ingredientes
    ...
    // Criar os ingredientes com referência ao produto correto
    await tx.productIngredient.createMany({
      data: products.flatMap((product) => {
        const productId = createdProducts.find(
